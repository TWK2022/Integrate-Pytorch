...
class OD(object):
    ...
    def _train(self,dict_dataset,model,loss):
        ...
        for epoch in range(1, args.epoch + 1):
            ...
            train_loader=train_get_loader(args,dict_dataset)
            for item, (train_batch, mask_batch, true_batch, name_batch, gt_number_batch) in enumerate(train_loader):
                ...
                if args.train_show[0]:
                    if (item+1) % args.train_show[1] == 0:
                        accuracy, precision, recall = metric.nmsbefore_acc_pre_recall(
                            pred_batch,mask_batch,true_batch,args.OD_screen_confidence,args.OD_screen_iou)
                        print('| {} | 迭代次数:{} | 本批量loss({}): 边框_{:.4f} 置信度_{:.4f} 分类_{:.4f} '
                              '| 非极大值抑制前指标: 置信度阈值_{} iou阈值_{} 准确率_{:.4f} 精确率_{:.4f} 召回率_{:.4f} |'
                              .format(args.model, item + 1, args.loss, loss1_item, loss2_item, loss3_item,
                                      args.OD_screen_confidence, args.OD_screen_iou, accuracy, precision, recall))
            accuracy, precision, recall = metric.nmsbefore_acc_pre_recall(
                pred_batch,mask_batch,true_batch,args.OD_screen_confidence,args.OD_screen_iou)
...
class OD_dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        train=torch.tensor(dict_dataset['img_train'][index]/255).type(torch.float32).to(args.device)  # 归一化
        ...
        for i,stride in enumerate(dict_dataset['stride']):  # 对每个输出层分别处理
            label_train = torch.tensor(dict_dataset['label_train'][index][2]).to(args.device)
            ...
            list_mask[i] = mask
            list_label[i] = label
            name = dict_dataset['label_train'][index][0]
            gt_number = dict_dataset['label_train'][index][1]
        return train,list_mask,list_label,name,gt_number
